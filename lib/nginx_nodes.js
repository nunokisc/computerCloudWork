...
module.exports = {
	...
	getNonTimeOutDel: function(callback)
	{
		...
		if(timeOutDel.length > 0)
		{
			...
			timeOutDel:
			for (let i = 0; i < timeOutDel.length; i++) 
			{
				console.log(i);
				for (let l = 0; l < onlineNginxNodeContainers.length; l++) 
				{
					console.log(l);
					if(timeOutDel[i].name != onlineNginxNodeContainers[l].name)
					{
						nginxNode = onlineNginxNodeContainers[l];
