...
var rootContainers = {};
var reservedIps = [];
// solução temporaria para dar expose a portas dos lb's
var temporario = 0;
//----------------------------------------------------
module.exports = {
	docker: docker,
	startContainer: function (containerType, containerName, containerBinds, containerIp, callback)
	{
		...
		else if(containerType == 'mysql-server')
		{
			...
			docker.createContainer({Image: 'percona:5.6', Cmd: ['mysqld'], name: containerName, Env:['MYSQL_ROOT_PASSWORD=root','MYSQL_DATABASE=wordpress'], HostConfig: {'Binds': containerBinds}, NetworkingConfig: { "EndpointsConfig": { "br0": { "IPAMConfig": { "IPv4Address": containerIp} } } }}, function (err, container) {
				if(err)
					console.log(err);
		...
		{
			if(containerName.includes('cwc-nginxlb-'))
			{
				// solução temporaria para dar expose a portas dos lb's
				let ExposedPorts = {};
				ExposedPorts["8"+temporario+"/tcp"] = {};
				module.exports.getLoadBalancerContainerId(function(e){
					temporario = e.length;
				})
				//----------------------------------------------------

				docker.createContainer({Image: 'kisc/nginx-telegraf', Cmd: ['/bin/bash', '-c', "service telegraf start ; mkdir -p /var/cache/nginx/fpc ; nginx -g 'daemon off;'"], name: containerName, ExposedPorts, HostConfig: {'Binds': containerBinds, "PortBindings": { "80/tcp": [{ "HostPort": "8"+temporario }] }}, NetworkingConfig: { "EndpointsConfig": { "br0": { "IPAMConfig": { "IPv4Address": containerIp} } } }}, function (err, container) {
					if(err)
						console.log(err);
					container.start(function (err, data) 
					{
	...
	setReservedIps: function(_reservedIps)
	{
		usedIps = usedIps.concat(_reservedIps);
		reservedIps = _reservedIps;
	},
	getReservedIps: function(callback)
	{
		return callback(reservedIps);
	},
	...
	getLoadBalancerContainerId: function(callback)
	{
		...
		{
			if(onlineContainers[i].name.includes('cwc-nginxlb-'))
			{
				nginxlbId.push(onlineContainers[i]);
			}
		}
